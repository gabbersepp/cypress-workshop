...
export async function post(url, data) {
    let result = await fetch(url, {
        ...
        headers: {
            "Authorization": "Bearer " + JSON.parse(localStorage.getItem("demoreference_access_token")).accessToken,
...
export async function createTicket(searchstring) {
    let ticket = {
        ...
        entranceType: "Default.DataSources.EntranceTypes.Phone",
        language: "Default.DataSources.SupportLanguages.En",
...
export function provideMetadata(data, identifier) {
    return post(`https://demo.firstanswer.de/api/AutomaticTestSupport/ProvideCustomMetadata?identifier=${identifier}`, data);
}